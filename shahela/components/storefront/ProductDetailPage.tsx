...
export function ProductDetailPage({ product }: { product: ProductCardModel }) {
  ...
  const addToCart = () => {
    addItem({ id: `cart-${product.id}-${activeVariant || 'default'}`, productId: product.id, variantId: variant?.id || null, name: product.name, variantName: variant?.name || null, price, imageUrl: product.imageUrl, quantity, customNote: null, leadTimeDays: variant?.leadTimeDays ?? product.leadTimeDays })
    setAdded(true)
