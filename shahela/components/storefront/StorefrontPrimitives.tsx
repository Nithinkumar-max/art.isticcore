...
export function ProductCard({ product, compact = false }: { product: ProductCardModel; compact?: boolean }) {
  ...
  return (
    <article className={`group flex min-w-0 flex-col overflow-hidden rounded-[28px] border border-surface-container-high bg-surface-container-lowest p-2.5 transition duration-300 hover:-translate-y-1 hover:shadow-[0_20px_42px_rgba(172,42,93,0.12)] ${compact ? 'sm:flex-row sm:gap-5 sm:p-4' : 'soft-shadow'}`}>
      <div className={`relative shrink-0 overflow-hidden rounded-[22px] bg-surface-container-low ${compact ? 'aspect-square w-full sm:h-36 sm:w-36' : 'aspect-[4/5] w-full'}`}>
        ...
        <img src={product.imageUrl} alt={`${product.name}, handcrafted crochet product`} loading="lazy" className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
